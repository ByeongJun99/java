...
//	}
	   ...
	   // 2. 내 pc DB상에 JDBC 계정에 TEST테이블의 모든 데이터 조회해보기
	   ...
	   try {
		   ...
		   // rset.next() => rset의 다음 행이 들어있는지 없는지 확인 후 결과 반환 + 한 행을 내려준다.
		   
		   while (rset.next()) {
			   // 현재 참조하는 rset으로부터 어떤 컬럼에 해당하는 값을 어떤 타입으로 추출할 것인지 제시해줘야함.
			   // DB의 컬럼명 제시(대소문자 구분x)
