...
public class Run {
	 ...
	 *  - 런타임에러 : 코드 상으로 문제가 없는데 프로그램 실행 중에 발생하는 에러(사용자의 실수일 수도 있고 개발자가 예측 가능하지만 처리를 안해놓은 에러)
	 *  - 논리에러 : 문법적으로도 문제없고 실행했을 때도 문제는 없지만 프로그램 의도와 맞지 않는 동작이나 결과가 나타나는 에러
	 *  
	 ...
	 * 
	 * * 예외처리를 하는 목적
	 *  - 예외처리를 하지않고 그대로 예외가 발생되는 경우, 프로그래밍 비정상적으로 종료될 수 있음
	 *  
	 * * 예외처리 방법
	 * 	1. try-catch문을 이용
...
//		}
		...
		// finally : try-catch의 결과와 상관없이 try-catch의 모든 과정이 끝났을때 무조건 실행하는 구문
