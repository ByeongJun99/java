...
public class TCPServer {
	 ...
	 *    - 데이터를 교환하기에 앞서 서버, 클라이언트 연결이 되어야한다.(서버가 먼저 실행되어 클라이언트의 요청을 기다림)
	 *    - 신뢰성 있는 데이터 전달 가능
	 *    
	 *  * Socket
	 *    - 프로세스 간의 통신을 담당
	 *    - Input / OutputStream을 가지고 있다. (이 스트림을 통해서 입출력이 이루어진다.)		-> 1바이트만 됨
	 *    
	 *  * ServerSocket
	...
	public static void main(String[] args) {
		...
		// 1) 포트번호 지정(서버 측에서 몇 번 포트로 통로를 열겠냐라는 의미)
		int port = 3000;
		...
		try {
			...
			// 5) 클라이언트와 입출력 기반 스트림 생성 (바이트스트림밖에 안됨)
			// 6) 보조스트림을 통한 성능개선 (바이트스트림밖에 안되기 때문)
